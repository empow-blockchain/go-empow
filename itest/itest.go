	...
	go func() {
		...
		for i := 0; i < num; i++ {
			...
			go func(res chan interface{}) {
				...
				err := t.Transfer(A, B, "em", fmt.Sprintf("%0.8f", amount), memoSize, check)
...
func (t *ITest) BuyRAMN(actionType string, num int, accounts []*Account, check bool) (successNum int, firstErr error) {
	...
	AmountLimit = []*contract.Amount{{Token: "em", Val: "1000"}, {Token: "ram", Val: "1000"}}
...
func (t *ITest) ExchangeTransfer(sender, recipient *Account, amount string, memoSize int, check bool) error {
	...
	err := client.ExchangeTransfer(sender, recipient, "em", amount, memoSize, check)
	if err != nil {
		return err
	}

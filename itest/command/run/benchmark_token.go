...
var BenchmarkTokenAction = func(c *cli.Context) error {
	...
	for _, acc := range accounts {
		tokenMap["em"].balance[acc.ID] = acc.Balance()
		tokenMap["em"].acclist = append(tokenMap["em"].acclist, acc.ID)
	}
			...
			for item := range hashCh {
				...
				if err != nil {
					...
					for i := 0; i < len(r.Receipts); i++ {
						if r.Receipts[i].FuncName == "token.empow/issue" {
							args := make([]string, 3)
							...
							tokenMutex.Unlock()
							break
						} else if r.Receipts[i].FuncName == "token.empow/create" {
	...
	for {
		...
		for num := 0; num < tps; num++ {
			...
			case tIndex <= 0 || len(tokenList) < 5:
				...
				act0 := tx.NewAction("ram.empow", "buy", fmt.Sprintf(`["%v", "%v", %v]`, "EM2ZsSw7RWYC229Z1ib7ujKhken9GFR7dBkTTEbBWMKeLpVas", from.ID, 10000))
				act1 := tx.NewAction("gas.empow", "pledge", fmt.Sprintf(`["%v", "%v", "%v"]`, "EM2ZsSw7RWYC229Z1ib7ujKhken9GFR7dBkTTEbBWMKeLpVas", from.ID, 10))
			...
			case tIndex <= 1000:
				...
				act0 := tx.NewAction("ram.empow", "buy", fmt.Sprintf(`["%v", "%v", %v]`, "EM2ZsSw7RWYC229Z1ib7ujKhken9GFR7dBkTTEbBWMKeLpVas", issuer.ID, 1000))
				act1 := tx.NewAction("gas.empow", "pledge", fmt.Sprintf(`["%v", "%v", "%v"]`, "EM2ZsSw7RWYC229Z1ib7ujKhken9GFR7dBkTTEbBWMKeLpVas", issuer.ID, 10))
				tx0 := itest.NewTransaction([]*tx.Action{act0, act1})
				trx, err := it.GetDefaultAccount().Sign(tx0)
				if err != nil {
					errList = append(errList, err)
			...
			default:
				...
				to := accounts[rand.Intn(len(accounts))]
				balance := tokenMap[tokenSym].balance[from.ID]
				act0 := tx.NewAction("ram.empow", "buy", fmt.Sprintf(`["%v", "%v", %v]`, "EM2ZsSw7RWYC229Z1ib7ujKhken9GFR7dBkTTEbBWMKeLpVas", from.ID, 100))
				act1 := tx.NewAction("gas.empow", "pledge", fmt.Sprintf(`["%v", "%v", "%v"]`, "EM2ZsSw7RWYC229Z1ib7ujKhken9GFR7dBkTTEbBWMKeLpVas", from.ID, 10))
				tx0 := itest.NewTransaction([]*tx.Action{act0, act1})
				trx, err := it.GetDefaultAccount().Sign(tx0)
				if err != nil {

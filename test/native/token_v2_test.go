...
func TestTokenV2_Create(t *testing.T) {
	...
	code.ID = "token.empow"
	host.Context().Set("contract_name", "token.empow")
	...
	Convey("Test of Token create", t, func() {
		...
		Convey("token not exists", func() {
			...
			_, _, err := e.LoadAndCall(host, code, "issue", "em", "user0", "100")
			So(err.Error(), ShouldEqual, "token not exists")

			_, _, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "100", "")
			So(err.Error(), ShouldEqual, "token not exists")

			_, _, err = e.LoadAndCall(host, code, "balanceOf", "em", "issuer0")
			So(err.Error(), ShouldEqual, "token not exists")

			_, _, err = e.LoadAndCall(host, code, "supply", "em")
			So(err.Error(), ShouldEqual, "token not exists")

			_, _, err = e.LoadAndCall(host, code, "totalSupply", "em")
			So(err.Error(), ShouldEqual, "token not exists")
		})
		...
		Convey("create token config", func() {
			authList[issuer0] = 1
			host.Context().Set("auth_list", authList)
			_, _, err := e.LoadAndCall(host, code, "create", "em", "issuer0", int64(100), []byte(`{"canTransfer": false, "decimal": 1, "defaultRate": "1.1"}`))
			So(err, ShouldBeNil)

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "10.222")
			So(err, ShouldBeNil)
...
func TestTokenV2_Issue(t *testing.T) {
	...
	Convey("Test of Token issue", t, func() {
		...
		Convey("correct issue", func() {
			_, cost, err := e.LoadAndCall(host, code, "issue", "em", "user0", "1.1")
			So(err, ShouldBeNil)
			...
			rs, cost, err = e.LoadAndCall(host, code, "supply", "em")
			So(err, ShouldBeNil)
			So(true, ShouldEqual, len(rs) > 0 && rs[0] == "1.1")

			rs, cost, err = e.LoadAndCall(host, code, "totalSupply", "em")
			So(err, ShouldBeNil)
		...
		Convey("issue token without auth", func() {
			...
			host.Context().Set("auth_list", authList)
			_, _, err := e.LoadAndCall(host, code, "issue", "em", "user0", "1.1")
		...
		Convey("issue too much", func() {
			...
			_, cost, err = e.LoadAndCall(host, code, "issue", "em", "user0", "100")
		...
		Convey("issue invalid amount", func() {
			_, cost, err := e.LoadAndCall(host, code, "issue", "em", "issuer0", "-1.1")
			So(true, ShouldEqual, strings.Contains(err.Error(), "invalid amount"))
			So(cost.ToGas(), ShouldBeGreaterThan, 0)

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "1.1")
			So(err, ShouldBeNil)

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "1abc")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "11111111111111111111111111111111")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "1.012345600000007e1")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", ".012345600000007e1")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "1.")
			So(err.Error(), ShouldContainSubstring, "invalid")
		})
...
func TestTokenV2_Transfer(t *testing.T) {
	...
	Convey("Test of Token transfer", t, func() {
		...
		Reset(func() {
			...
			_, _, err = e.LoadAndCall(host, code, "issue", "em", "issuer0", "100.0")
			So(err, ShouldBeNil)
		})
		...
		Convey("correct transfer", func() {
			_, cost, err := e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "22.3", "")
			So(err, ShouldBeNil)
			So(cost.ToGas(), ShouldBeGreaterThan, 0)
			...
			authList["user0"] = 1
			_, cost, err = e.LoadAndCall(host, code, "transfer", "em", "user0", "user1", "22.3", "")
			So(err, ShouldBeNil)
			...
			// transfer to self
			authList["user1"] = 1
			_, cost, err = e.LoadAndCall(host, code, "transfer", "em", "user1", "user1", "22.3", "")
			So(err, ShouldBeNil)
		...
		Convey("transfer token without auth", func() {
			delete(authList, issuer0)
			host.Context().Set("auth_list", authList)
			_, cost, err := e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "1.1", "")
			So(true, ShouldEqual, err.Error() == "transaction has no permission")
			So(cost.ToGas(), ShouldBeGreaterThan, 0)
		})

		Convey("transfer too much", func() {
			_, cost, err := e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "100.1", "")
		...
		Convey("transfer invalid amount", func() {
			...
			_, cost, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, cost, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "1abc", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, cost, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "11111111111111111111111111111111", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "1.012345600000007e1", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", ".012345600000007e1", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "transfer", "em", "issuer0", "user0", "1.", "")
			So(err.Error(), ShouldContainSubstring, "invalid")
		})
	})
}
...
func TestTokenV2_Destroy(t *testing.T) {
	...
	Convey("Test of Token destroy", t, func() {
		...
		Convey("correct destroy", func() {
			...
			_, cost, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", "22.3")
			So(err, ShouldBeNil)
		...
		Convey("issuer after destroy", func() {
			...
			_, cost, err = e.LoadAndCall(host, code, "issue", "em", "user0", "11")
			So(err, ShouldBeNil)
			...
			_, cost, err = e.LoadAndCall(host, code, "issue", "em", "user0", "21")
			So(true, ShouldEqual, err.Error() == "supply too much")
		})
		...
		Convey("destroy token without auth", func() {
			delete(authList, issuer0)
			host.Context().Set("auth_list", authList)
			_, _, err := e.LoadAndCall(host, code, "destroy", "em", "issuer0", "1.1")
			So(true, ShouldEqual, err.Error() == "transaction has no permission")
		})

		Convey("destroy too much", func() {
			_, cost, err := e.LoadAndCall(host, code, "destroy", "em", "issuer0", "100.1")
			So(true, ShouldEqual, strings.HasPrefix(err.Error(), "balance not enough"))
		...
		Convey("destroy invalid amount", func() {
			_, cost, err := e.LoadAndCall(host, code, "destroy", "em", "issuer0", "-1.1")
			So(true, ShouldEqual, strings.Contains(err.Error(), "invalid amount"))
			So(cost.ToGas(), ShouldBeGreaterThan, 0)
			...
			_, cost, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, cost, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", "1abc")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, cost, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", "11111111111111111111111111111111")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", "1.012345600000007e1")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", ".012345600000007e1")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "destroy", "em", "issuer0", "1.")
			So(err.Error(), ShouldContainSubstring, "invalid")
		})
	})
}
...
func TestTokenV2_TransferFreeze(t *testing.T) {
	...
	Convey("Test of Token transferFreeze", t, func() {
		...
		Convey("correct transferFreeze", func() {
			_, cost, err := e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", "22.3", now, "")
			So(err, ShouldBeNil)
			So(cost.ToGas(), ShouldBeGreaterThan, 0)
			...
			freezedBalance := host.DB().FreezedTokenBalanceFixed("em", "user0")
			...
			authList["user0"] = 1
			_, cost, err = e.LoadAndCall(host, code, "transferFreeze", "em", "user0", "user0", "10", now+10, "")
			So(err, ShouldBeNil)
			So(cost.ToGas(), ShouldBeGreaterThan, 0)
			...
			_, cost, err = e.LoadAndCall(host, code, "transferFreeze", "em", "user0", "user0", "1", now+20, "")
			So(err, ShouldBeNil)
		...
		Convey("transferFreeze token without auth", func() {
			delete(authList, issuer0)
			host.Context().Set("auth_list", authList)
			_, _, err := e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", "1.1", now, "")
			So(true, ShouldEqual, err.Error() == "transaction has no permission")
		})

		Convey("transferFreeze too much", func() {
			_, _, err := e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", "100.1", now-1, "")
			So(true, ShouldEqual, strings.HasPrefix(err.Error(), "balance not enough"))
			...
			rs, _, err = e.LoadAndCall(host, code, "balanceOf", "em", "user0")
			So(err, ShouldBeNil)
			So(true, ShouldEqual, len(rs) > 0 && rs[0] == "0")

			_, _, err = e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", "100", now+100, "")
			So(err, ShouldBeNil)

			authList["user0"] = 1
			host.Context().Set("auth_list", authList)
			_, _, err = e.LoadAndCall(host, code, "transferFreeze", "em", "user0", "user1", "10", now+100, "")
			So(true, ShouldEqual, strings.HasPrefix(err.Error(), "balance not enough"))

			_, _, err = e.LoadAndCall(host, code, "transfer", "em", "user0", "user1", "10", "")
			So(true, ShouldEqual, strings.HasPrefix(err.Error(), "balance not enough"))
		})

		Convey("transferFreeze invalid amount", func() {
			_, _, err := e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", "1.012345600000007e1", now, "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", ".012345600000007e1", now, "")
			So(err.Error(), ShouldContainSubstring, "invalid")

			_, _, err = e.LoadAndCall(host, code, "transferFreeze", "em", "issuer0", "user0", "1.", now, "")
			So(err.Error(), ShouldContainSubstring, "invalid")
		})

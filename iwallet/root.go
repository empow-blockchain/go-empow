...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&accountName, "address", "a", "", "which address to use")
	rootCmd.PersistentFlags().StringVarP(&accountDir, "address_dir", "", "", "$(address_dir)/.iwallet will be used to save addresses (default $HOME/.iwallet)")
